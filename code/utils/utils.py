...
from keras.models import Model, load_model, model_from_json, model_from_config, Sequential
from keras import optimizers
from utils.kmgpu import multi_gpu_model
...
def cal_dist(var_csv,anno):
    ...
    for index_csv,row_csv in var_csv.iterrows():
        ...
        for index_ann,row_ann in mini_ann.iterrows():                   
            ...
            #print(np.max(np.array([a1,a2,a3]))/row_ann['diameter_mm'])
...
z = z position of slice in world coordinates mm
    ...
    v_xmin = np.max([0, int(v_center[0] - v_diam) - 0])
    v_xmax = np.min([width - 1, int(v_center[0] + v_diam) + 0])
    v_ymin = np.max([0, int(v_center[1] - v_diam) - 0])
    v_ymax = np.min([height - 1, int(v_center[1] + v_diam) + 0])
...
def pred_tests(data_path,img_file,preded_path):
    patient_id = img_file.split('/')[-1][:-4]
    itk_img = SimpleITK.ReadImage(data_path + img_file) 
    img_array = SimpleITK.GetArrayFromImage(itk_img) # inde
    origin = np.array(itk_img.GetOrigin())      # x,y,z  Origin in world coordinates (mm)
