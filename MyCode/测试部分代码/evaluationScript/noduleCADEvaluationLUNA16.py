...
# plot settings
FROC_minX = 0.125 # FROC曲线的x轴的最小值
FROC_maxX = 8     # FROC曲线的x轴的最大值
bLogPlot = True
...
def generateBootstrapSet(scanToCandidatesDict, FROCImList):
    ...
    # 使用替换采样获取随机图像列表
    rand_index_im   = np.random.randint(imageLen, size=imageLen) #产生矩阵
...
def computeFROC(FROCGTList, FROCProbList, totalNumberOfImages, excludeList):
    ...
    if sum(FROCGTList) == len(FROCGTList): # 当没有误报时处理边界情况并且ROC分析给出nan值.
...
def noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename,outputDir):
    '''
    用于加载注释和评估CAD算法的功能
    @param annotations_filename: 标记列表
    @param annotations_excluded_filename: 从分析中排除的注释列表
    @param seriesuids_filename: 病人id的ct图像列表
    @param results_filename: 带有概率的CAD标记列表
    @param outputDir: 输出目录
    '''
...
if __name__ == '__main__':
    ...
    noduleCADEvaluation('/Volumes/solo/ali/evaluationScript/annotations/annotations_all.csv',#原始的csv文件
                        '/Volumes/solo/ali/evaluationScript/annotations/annotations_excluded.csv',#每列数据代表的数据
                        '/Volumes/solo/ali/evaluationScript/annotations/seriesuids.csv',#每个结节的病人id
